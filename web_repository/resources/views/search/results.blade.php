...
<head>
    ...
    <style>
        * {
        ...
        /* Admin & Collaboration - 3 Columns */
        .file-item.administration,
        .file-item.collaboration {
            grid-template-columns: 2fr 1fr 2fr auto;
        }
        ...
        .admin-title .collab-title .course-title{
            font-size: clamp(14px, 2vw, 16px);
        }
        ...
        @media (max-width: 1023px) {
            ...
            /* Research - 1 Column */
            .file-item.research, .file-item.course, .file-item.admin,
            .file-item.collaboration{
        ...
        @else
            <ul class="file-list">
                @foreach($paginatedResults as $result)
                    <li class="file-item @if($result->category === 'administration') administration
                                         @elseif($result->category === 'collaboration') collaboration 
                                         @elseif($result->category === 'research') research 
                                         @elseif($result->category === 'course') course 
                        ...
                        @if($result->category === 'administration')
                            ...
                            <p>{{ $result->type }}</p>
                            <button onclick="window.location.href='{{ route('file.download', ['category' => 'administration', 'id' => $result->id]) }}'">Download</button>
                        
                        @elseif($result->category === 'collaboration')
                        ...
                        @elseif($result->category === 'research')
                            ...
                            <div class="research-details">
                                Peneliti: {{ $result->researcher_name ?? 'Unknown' }} | Tahun: {{ $result->year ?? 'Unknown' }}
                            </div>
                            <p>{{ $result->topic ?? 'No Topic Available' }}</p>
                            <button onclick="window.location.href='{{ route('file.download', ['category' => 'research', 'id' => $result->id]) }}'">Download</button>
                            
                        @elseif($result->category === 'course')
                            ...
                            <p>{{ $result->subject ?? 'Unknown Subject' }}</p>
                            <p>{{ $result->class ?? 'Unknown Class' }}</p>
                            <button onclick="window.location.href='{{ route('file.download', ['category' => 'course', 'id' => $result->id]) }}'">Download</button>    
                        
                        @else
                            {{-- Default display if no specific category --}}
                            <strong>{{ $result->file_name }}</strong>
                            <p>{{ $result->description ?? 'No description available' }}</p>
                            <button onclick="window.location.href='{{ route('file.download', ['category' => 'unknown', 'id' => $result->id]) }}'">Download</button>
                        @endif
                    </li>
                @endforeach
            </ul>
