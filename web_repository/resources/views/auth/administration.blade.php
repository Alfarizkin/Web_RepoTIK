...
<head>
    ...
    <style>
        * {
        ...
        .file-item.admin {
            grid-template-columns: 2fr 2fr auto; /* Judul, Tipe/Proyek, Tombol */
        }
...
<body>
    ...
    <div class="content">
        ...
        @if($paginatedAdmins->isEmpty())
            <p class="no-result">No Files found.</p>
        @else
            <ul class="file-list"> 
                @foreach($paginatedAdmins as $result)
                    <li class="file-item admin">
                        ...
                        <p>{{ $result->type }}</p>
                        <button onclick="window.location.href='{{ route('file.download', ['category' => 'admin', 'id' => $result->id]) }}'">Download</button>
                    </li>
                @endforeach
            </ul>
            ...
            <div class="pagination-container">
                <div class="pagination-top">
                    <div>
                        <p class="small text-muted">
                            {!! __('Showing') !!}
                            <span class="fw-semibold">{{ $paginatedAdmins->firstItem() }}</span>
                            {!! __('to') !!}
                            <span class="fw-semibold">{{ $paginatedAdmins->lastItem() }}</span>
                            {!! __('of') !!}
                            <span class="fw-semibold">{{ $paginatedAdmins->total() }}</span>
                            {!! __('results') !!}
                        </p>
                    </div>
                </div>
                ...
                <div class="pagination-bottom">
                    {{ $paginatedAdmins->links('pagination::bootstrap-5') }}
                </div>   
            </div>
        @endif
    </div>
